...
public class Editor extends javax.swing.JFrame {
	...
	public void menuFileAction(Object object){
		if(object == JMINewFile){
			if(getJTextAreaEditor().getText().length()>0){
			...
			if(JFChooser.showOpenDialog(null)==0){
				setPath(getFileAbsolutePath());
				if(new File(path).exists()){
					JTextAreaEditor.setText("");
					lerArquivo(path);
					if(path.lastIndexOf("/")!=-1)	
						setTitle("Simpleton Editor -"+path.substring(path.lastIndexOf("/")+1, path.length()));
					else
						setTitle("Simpleton Editor -"+path.substring(path.lastIndexOf("\\")+1, path.length()));
				}
				else
					menuFileAction(JMIOpen);
			...
			if(JFChooser.showSaveDialog(null) == 0){
				...
				if(tempPath!=null)
					if(new File(tempPath).exists()){
						if(JOptionPane.showConfirmDialog(null, "There exist a "+JFChooser.getSelectedFile().getName()+" file. Overwrite?") == 0)
							path = tempPath;
					}
					else
						path = tempPath;
			...
			if(JTextAreaEditor.getText().length()>0){
				if(JOptionPane.showConfirmDialog(null, "Save current work?")==0)
					menuFileAction(JMISave);
			}
			dispose();
			...
			if(!path.equals("")){
				output = new Output();
				if(path.lastIndexOf("/")!=-1)
					output.setWindowTitle(path.substring(path.lastIndexOf("/"))+1);
				else
					output.setWindowTitle(path.substring(path.lastIndexOf("\\"))+1);
